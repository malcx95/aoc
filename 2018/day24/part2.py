...
while True:
    ...
    while not is_over(new_groups):
        ...
        attacks = {}
        gets_attacked = set()
        groups_sorted = sorted(new_groups, key=lambda g: (g.get_effective_power(),
                                                   g.initiative),
                            reverse=True)
        ...
        # attacking phase
        for group in sorted(new_groups, key=lambda g: g.initiative, reverse=True):
    ...
    if new_groups[0].type == "Immune System":
        break
    else:
        print("Infection won, with", sum(g.num_units for g in new_groups),
             'units left')
        boost += 1

print("Immune system won! It has", sum(g.num_units for g in new_groups),
     'units left')
